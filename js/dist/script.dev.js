...
filmArray = mainArray.reduce(function (total, _ref, index) {
  var genre = _ref.genre;
  genre.forEach(function (el) {
    if (total.indexOf(el) === -1) {
      return [].concat(_toConsumableArray(total), [el]);
    }
  });
  return total;
}, []);
console.log(filmArray); // 2. Собрать в массив всех актеров всех фильмов (без повторения)
